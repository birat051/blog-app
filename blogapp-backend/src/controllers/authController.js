...
exports.signup = async (req, res) => {
  try {
    ...
    if (!password) {
      return res
        .status(400)
        .json({ message: 'Password is required in request body' });
    }
    ...
    res
      .status(500)
      .json({ message: 'Unexpected error occured while creating new user' });
  }
};
...
exports.signout = async (req, res) => {
  try {
    const token = req.header('Authorization');
    const tokenDbData = await JwtTokenModel.deleteOne({ token });
